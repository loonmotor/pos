...
var renderRoute = function renderRoute(req, res, routeObj) {
	var routeProps = getPropsFromRoute(routeObj, ['requestInitialData', 'noScriptPost']);
	if (routeProps.noScriptPost && req.method === 'POST') {
		routeProps.noScriptPost(req.body).then(function (data) {
	...
	if (routeProps.requestInitialData) {
		routeProps.requestInitialData({ server: { originalUrl: req.originalUrl.split('/'), query: req.query } }).then(function (data) {
			var handleCreateElement = function handleCreateElement(Component, props) {
				return _react2.default.createElement(Component, _extends({ initialData: data }, props));
			};
			res.render('index', {
